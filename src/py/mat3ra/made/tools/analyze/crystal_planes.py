...
class CrystalLatticePlanesMaterialAnalyzer(LatticeMaterialAnalyzer):
    ...
    def get_material_with_termination_without_vacuum(self, termination: Termination) -> Material:
        ...
        # Find the matching slab by comparing the termination formula
        slabs_without_vacuum = self.all_planes_as_pymatgen_slabs_without_vacuum
        matching_slab = next(
            (
                slab for slab in slabs_without_vacuum
                if Termination.from_string(label_pymatgen_slab_termination(slab)).formula 
                == holder.termination_without_vacuum.formula
            ),
            None,
        )
        
        if matching_slab is None:
            raise ValueError(f"No matching slab found for termination: {holder.termination_without_vacuum}")
            
        return Material.create(from_pymatgen(matching_slab))
