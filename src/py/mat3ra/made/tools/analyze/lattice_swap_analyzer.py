...
class MaterialLatticeSwapAnalyzer(BaseModel):
    ...
    def canonicalize_in_plane_material(
        ...
        for operation in cls._get_in_plane_symmetry_operations(material, tolerance):
            ...
            x_candidates = sorted({cls._normalize_fractional_coordinate(coord[0], tolerance) for coord in transformed_coordinates})
            y_candidates = sorted({cls._normalize_fractional_coordinate(coord[1], tolerance) for coord in transformed_coordinates})
    ...
    def _canonical_atom_key(cls, element, label, coordinate, tolerance: float):
        rounded_coordinate = tuple(round(cls._normalize_fractional_coordinate(value, tolerance), 8) for value in coordinate)
