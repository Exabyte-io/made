...
class PointDefectAnalyzer(BaseMaterialAnalyzer):
    ...
    def _resolve_to_crystal_site(self, coordinate: List[float], element: Optional[str] = None) -> List[float]:
        site_id = get_closest_site_id_from_coordinate(self.host_material, coordinate)
        return self.host_material.coordinates_array[site_id]
    ...
    def get_configuration(
        ...
        if defect_type == PointDefectTypeEnum.VACANCY:
            return VacancyDefectConfiguration.from_parameters(
                host_material=self.host_material, coordinate=resolved_coordinate
            )
            ...
            return SubstitutionalDefectConfiguration.from_parameters(
                host_material=self.host_material, coordinate=resolved_coordinate, element=element
            )
