...
def compare_materials_ignoring_z_shift(material1: Material, material2: Material, tolerance: float = 1e-6) -> bool:
    """
    Compare two materials while ignoring linear shifts in the z-direction.
    Both materials are normalized to have their lowest atom at z=0 before comparison.
    
    Args:
        material1: First material to compare.
        material2: Second material to compare.
        tolerance: Tolerance for coordinate comparison.
        
    Returns:
        bool: True if materials are equivalent (ignoring z-shift), False otherwise.
    """
    # Normalize both materials
    norm_mat1 = normalize_material_z_coordinates(material1)
    norm_mat2 = normalize_material_z_coordinates(material2)
    
    # Check lattice parameters
    if (abs(norm_mat1.lattice.a - norm_mat2.lattice.a) > tolerance or
        abs(norm_mat1.lattice.b - norm_mat2.lattice.b) > tolerance or
        abs(norm_mat1.lattice.c - norm_mat2.lattice.c) > tolerance):
        return False
    
    # Check number of atoms
    if len(norm_mat1.basis.coordinates.values) != len(norm_mat2.basis.coordinates.values):
        return False
    
    # Check elements
    if norm_mat1.basis.elements.values != norm_mat2.basis.elements.values:
        return False
    
    # Check coordinates (after normalization)
    for coord1, coord2 in zip(norm_mat1.basis.coordinates.values, norm_mat2.basis.coordinates.values):
        if (abs(coord1[0] - coord2[0]) > tolerance or
            abs(coord1[1] - coord2[1]) > tolerance or
...
def normalize_material_z_coordinates(material: Material, reference_z: float = 0.0) -> Material:
    """
    Normalize material coordinates by translating all atoms so the lowest atom is at reference_z.
    This allows comparison of materials regardless of linear shifts in z-direction.
    
    Args:
        material: Material to normalize.
        reference_z: Reference z-coordinate for the lowest atom (default: 0.0).
        
    Returns:
        Material: Normalized material with lowest atom at reference_z.
    """
    normalized_material = material.clone()
    
    # Convert to cartesian coordinates to work with actual positions
    normalized_material.basis.to_cartesian()
    
    # Find the minimum z-coordinate
    z_coordinates = [coord[2] for coord in normalized_material.basis.coordinates.values]
    min_z = min(z_coordinates) if z_coordinates else 0.0
    
    # Calculate translation needed to move lowest atom to reference_z
    z_translation = reference_z - min_z
    
    # Apply translation if needed
    if abs(z_translation) > 1e-10:  # Only translate if significant difference
        translate_by_vector(normalized_material, [0, 0, z_translation], use_cartesian_coordinates=True)
    
    # Convert back to crystal coordinates
    normalized_material.basis.to_crystal()
