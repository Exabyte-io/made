...
function getMillerScalingMatrix(cell, millerIndices, tol = 1e-8) {
    ...
    if ((h0 && k0) || (h0 && l0) || (k0 && l0)) {  // if any two indices are zero

        if (!h0) scalingMatrix = [[0, 1, 0], [0, 0, 1], [1, 0, 0]];
        if (!k0) scalingMatrix = [[0, 0, 1], [1, 0, 0], [0, 1, 0]];
        if (!l0) scalingMatrix = [[1, 0, 0], [0, 1, 0], [0, 0, 1]];
        ...
        if (math.abs(k2) > tol) {
            const i = -parseInt(math.round(k1 / k2));
            [p, q] = [p + i * l, q - i * k];
        }
...
function generateConfig(material, millerIndices, numberOfLayers = 1, vx = 1, vy = 1) {
    ...
    const newBasis = SupercellTools.generateNewBasis(material.Basis, cell, supercell);
